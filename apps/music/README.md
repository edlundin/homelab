...
This namespace runs qBittorrent, Prowlarr, Lidarr, and Navidrome. All four
applications run as UID/GID `1000`. The NFS export must therefore have the
...
2. Open Prowlarr at <https://prowlarr.ison-mirfak.ts.net>. Add indexers, then
   add the Lidarr application at
...
The services are ClusterIP-only. qBittorrent peer ports are not exposed by a
Service. Digarr and SUB/WAVE are documented in their own operator README.
