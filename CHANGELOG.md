...
- **fix**: Remove MusicGrabber API-key authentication because its v4.0.2 browser client does not send the required header, which restores settings load and save through the private Tailscale route. ([pending](https://github.com/edlundin/homelab/commit/pending))
